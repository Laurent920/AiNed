"""
sepi_tmlr_jax_syncL1_1006.py
13062026
─────────────────────────────────────────────────────────────────────────────
Frame-based N-MNIST inference — fixed architecture: rank 1 synchronous LIF,
all other hidden ranks asynchronous.  Simplified from sepi_tmlr_jax_1006.py.

Architecture
────────────
N MPI ranks form a pipeline, one rank per layer:
  rank 0        — input feeder: reads event frames, sends (neuron_idx, value) pairs
  rank 1        — sync LIF: accumulates all frame events, fires once at each -3
...
Extends async_MLP.py predict() with:
  ...
  5. Rank 1 always synchronous  →  fires once per frame at -3 boundary

Run (adjust -n to match len(layer_sizes)):
  JAX_PLATFORMS=cpu mpirun -n <N> python sepi_tmlr_jax_syncL1_1006.py \\
      --config sepi_tmlr_conf_1006.yaml [--max_samples N]

Accuracy on full 10k N-MNIST test set, fgs=128, momentum, use_refrac=True:
  [2312, 128, 10]        sync-layer1+async : 96.29%
  [2312, 64, 64, 64, 10] sync-layer1+async : 91.81%
"""
...
def predict(params, key, weights, empty_neuron_states, batch_data, grad=False):
    ...
    Each rank runs a different inner function determined at trace time:
      ...
      rank 1        →  sync_lif_layer (synchronous: fires once per frame at -3)
      rank 2…N-2    →  other_layers   (async hidden layers)
    ...
    def sync_lif_layer(args):
        """
        Synchronous LIF processing for rank 1 (all frames).
        ...
        Firing order: fire on raw accumulated membrane (pre-decay),
    ...
    def loop_over_batches(_, x):
        ...
        Dispatches to the correct inner function based on layer_idx.
        The dispatch is a Python-level if-else (not jax.lax.cond) because
        layer_idx is a static Python int — only one branch is traced per rank,
        ...
        # layer_idx is static → only one branch is compiled per rank; no runtime cost.
        if layer_idx == 0:
            result = input_layer(args)
        elif layer_idx == 1:
            result = sync_lif_layer(args)   # rank 1 always synchronous
        else:
            result = other_layers(args)     # ranks 2…N-1 always async
...
def evaluate(cfg, max_samples=None, scheduler="momentum", return_stats=False):
    ...
    Rank 1 always uses synchronous LIF (fires once per frame at -3).
    Ranks 2…N-2 always use asynchronous LIF (fire every forward_group_size events).
...
def main():
    ...
    --fgs             : int  — override forward_group_size from config

    MPI layout (set from comm size and config layer_sizes)
    ──────────────────────────────────────────────────────
    rank 0        → input feeder
    rank 1        → sync LIF (fires once per frame at -3)
    rank 2…N-2    → async LIF (fires every forward_group_size events or at -3)
