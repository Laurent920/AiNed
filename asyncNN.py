...
def train(key, layer_sizes, thresholds, num_epochs, learning_rate, batch_size):    
    ...
    training_generator, train, test = torch_loader(batch_size, n_targets=10)
        ...
        for batch_x, y in training_generator:
            for x in batch_x:
                ...
                if rank == 0:
                    ...
                    neuron_states = layer_computation(activations, weights, neuron_states)                
            ...
            if rank == 2:
                ...
                output = neuron_states.values - jax.scipy.special.logsumexp(neuron_states.values)
...
def init_params(key):# Initialize weights for each layer
    ...
    if rank == 0:
        training_generator, train, test = torch_loader(batch_size, n_targets=10)
        # x = jax.random.normal(keys[0], layer_sizes[0])
        return training_generator, train, test
    else:
