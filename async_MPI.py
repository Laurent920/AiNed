...
# from mnist_helper import torch_loader, 
...
def predict(weights, neuron_states, token, batch_data=None):
    @jax.jit
    ...
    for batch_nb in range(batch_size):
        if rank == 0:
            ...
            # TODO Preprocess the data to only send the non-zero values
...
def init_params(key):# Initialize weights for each layer
    ...
    if rank != 0:
        ...
        print(f"weights shapes: {jnp.shape(weights)}, {weights}")
...
if __name__ == "__main__":
    ...
    if rank == 0:
        # Preprocess the data 
        training_generator, train_set, test_set, total_batches = torch_loader(batch_size)
    ...
    for epoch in range(num_epochs):
        for i in range(total_batches):
            ...
            if rank == 0:
                ...
                batch_x, batch_y = next(batch_iterator)
                # print(f"Batch_y: {batch_y}")
                token = send(batch_y, dest=size-1, tag=1,comm=comm)
