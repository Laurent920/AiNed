...
def activation_func_jvp(primals, tangents, k=1.0):
    ...
    u_od = activations - neuron_states.threshold 
    ans_dot = (1 / (k * jnp.abs(u_od) + 1) ** 2) * activations_dot
    return ans, ans_dot
...
def train(key, layer_sizes, thresholds, num_epochs, learning_rate, batch_size):    
    ...
    if rank != 0:
        ...
        neuron_states = Neuron_states(values=jnp.zeros((layer_sizes[0])), threshold=0)
