...
# LIF settings for the TMLR-style upgrade.
# This value is overwritten from YAML config key `tau_m` in main() if present.
LIF_TAU_M = 100.0
...
# region INFERENCE
# region TMLR-style LIF helper functions
# These helpers implement the LIF update described in the TMLR paper.
# Important: in Laurent's MPI loop, `iteration` inside forward_pass is used as
# the event time / micro-time / forward-step index.
def lif_decay(values, last_update_iteration, iteration):
    """
    NeuronDecay from the TMLR paper.

    u(t) = u(t_last) * exp(-(t - t_last) / tau_m)

    values:
        Current membrane potentials of this layer.
    last_update_iteration:
        Last event-time at which each neuron state was updated.
        In this file we reuse neuron_states.last_sent_iteration for that role.
    iteration:
        Current event-processing step in Laurent's forward_pass loop.
    """
...
def lif_input_current(params, neuron_idx, layer_input, weights):
    """
    InputLayerForward / synaptic-current computation.

    Laurent's event format is [previous_layer_neuron_index, value].
    This function converts one incoming event into a current vector for the
    current layer using the row of weights connected to that previous neuron.
    """
    valid_event = neuron_idx >= 0
    safe_neuron_idx = jnp.where(valid_event, neuron_idx, 0)

    filtered_weights = keep_top_k(
        weights[safe_neuron_idx],
        params.top_weights,
        apply_abs=True
    )

    input_current = layer_input * filtered_weights
    input_current = jax.lax.cond(
        valid_event,
        lambda _: input_current,
...
def lif_select_spikes(params, spikes):
    """
    Simplified SelectSpikes(s, F) for Laurent's layer-wise MPI framework.

    In the TMLR simulator, F is the forward group size. In this code,
    params.firing_nb is used as the local maximum number of outgoing spikes.
    """
    return keep_top_k(spikes, params.firing_nb)


def lif_neuron_forward(params, old_values, u_decayed, input_current, thresholds, valid_event, grad=False):
    """
    NeuronForward from the TMLR paper:
        1) integrate incoming current
        2) evaluate threshold immediately
        3) emit spike
        4) hard reset membrane to 0 after spike

    During training, we use a straight-through surrogate spike:
        forward value = hard binary spike
        backward gradient = activation_func surrogate
    """
    u_pre = u_decayed + input_current

    # Hard binary spike for the actual forward behavior.
    hard_spikes = (u_pre > thresholds).astype(jnp.float32)

    # Surrogate spike for gradient flow during training.
    surrogate_spikes = activation_func(thresholds, u_pre)

    # Straight-through estimator:
    # forward behaves like hard_spikes,
    # backward uses surrogate_spikes gradient.
    if grad:
        raw_spikes = surrogate_spikes + jax.lax.stop_gradient(hard_spikes - surrogate_spikes)
    else:
    ...
    # Reset should depend on actual hard spike, not the surrogate value.
    selected_hard_spikes = lif_select_spikes(params, hard_spikes)
    u_new = jnp.where(selected_hard_spikes > 0, 0.0, u_pre)

    # For the end signal (-1), keep the previous state unchanged.
    u_new = jax.lax.cond(
        valid_event,
        lambda _: u_new,
        lambda _: old_values,
...
def lif_network_layers_forward(params, key, selected_spikes):
    """
    NetworkLayersForward from the TMLR paper, adapted to this MPI code.

    Converts a binary spike vector into event messages [neuron_idx, value]
    that are later sent to the next MPI rank by predict().
    """
    ...
    processed_output = output_vector_to_event(
        ...
        selected_spikes,
        params,
        params.layer_sizes[layer_idx]
    )
    return valid_elements, processed_output


def lif_early_stop(params, selected_spikes):
    """
    Placeholder for the TMLR EarlyStop condition.

    For this first LIF upgrade, the code keeps Laurent's existing -1 end-signal
    stopping mechanism. A later version can stop when the output layer spikes.
    """
    return False
...
def layer_computation(params, key, neuron_idx, layer_input, weights, neuron_states, iteration=0, grad=False): 
        # jax.debug.print("rank {}, neuron idx {}", rank, neuron_idx)
    # Safe readout current for the last/output layer.
    # Hidden layers use the TMLR-style LIF logic inside hidden_layer_case().
    safe_neuron_idx_for_readout = jnp.where(neuron_idx >= 0, neuron_idx, 0)
    filtered_weights_for_readout = keep_top_k(
        weights[safe_neuron_idx_for_readout],
        params.top_weights,
        apply_abs=True
    )
    input_current_for_readout = jax.lax.cond(
        neuron_idx < 0,
        lambda _: jnp.zeros_like(neuron_states.values),
        lambda _: layer_input * filtered_weights_for_readout,
        None
    )
    activations = neuron_states.values + input_current_for_readout
    ...
    def hidden_layer_case(_):
        # ------------------------------------------------------------
        # TMLR-style asynchronous LIF update.
        # The `iteration` variable from Laurent's forward_pass loop is
        # used as event time / micro-time / forward-step index.
        # ------------------------------------------------------------

        # 1) InputLayerForward: incoming event -> synaptic current
        ...
        # 2) NeuronDecay: exponential leak before integrating the event
        u_decayed = lif_decay(
        ...
        # 3) NeuronForward: integrate -> threshold -> binary spike -> hard reset
        activated_output, new_values, _ = lif_neuron_forward(
            params=params,
        ...
        if grad:
            ...
            new_input_order = jax.lax.cond(
                ...
                lambda _: neuron_states.input_order,
                None
            )
            ...
            new_output_activity = jax.lax.cond(
                ...
                lambda _: neuron_states.output_activity,
                None
            )

            # Added +1 so that we can differentiate between never activated (0)
            # and activated at iteration 0 (1).
        ...
        # Reuse last_sent_iteration as last_update_iteration.
        # Because lif_decay() updates the whole membrane vector to the current
        # event time, all neurons receive the current iteration as last update.
        ...
        # 4) NetworkLayersForward: selected spike vector -> outgoing events
        valid_elements, processed_output = lif_network_layers_forward(
            params=params,
            key=key,
            selected_spikes=activated_output
        )
...
def main(random_seed, key, rank_, size_, comm_, trial=None, trial_params=None, config_path=None, data_dir=""):   
    ...
    # TMLR-style LIF membrane time constant.
    # Add `tau_m: 100.0` to your YAML config to override this default.
    global LIF_TAU_M
    ...
    for f_nb in [128]: # Loop for multiple experiments
        ...
        empty_neuron_states = NeuronStates( values=jnp.zeros(layer_sizes[layer_idx]),
                                            ...
                                            last_sent_iteration=jnp.zeros((layer_sizes[layer_idx],), dtype=jnp.int32), #make vector of the last sent iteration for each neuron to compute the inter-spike interval and use it for the firing condition
